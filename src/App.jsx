  ...
  return(
    <div style={{fontFamily:"'JetBrains Mono','Courier New',monospace",background:"#0a0a0f",color:"#c9d1d9",
      ...
      <header style={{background:"linear-gradient(90deg,#0d1117,#161b22)",borderBottom:"1px solid #21262d",
        padding:"9px 16px",display:"flex",alignItems:"center",gap:11,flexShrink:0}}>
        <div style={{color:"#58a6ff",fontWeight:700,fontSize:15,letterSpacing:3}}>M3W::EDITOR</div>
        <div style={{color:"#30363d",fontSize:10}}>v1.3</div>
        <div style={{flex:1}}/>
        ...
        {!simMode&&<>
          {[{key:"brush",label:"✏ BRUSH"},{key:"eraser",label:"⌫ ERASE"},{key:"void",label:"⬜ VOID"}].map(t=>(
            <button key={t.key} onClick={()=>setTool(t.key)} style={{
              ...
              padding:"5px 12px",fontSize:11,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"}}>{t.label}</button>
          ))}
          <Div/>
          {LAYERS.map(l=>(
            <button key={l} onClick={()=>setSelectedLayer(l)} style={{
              ...
              padding:"5px 10px",fontSize:11,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"}}>L{l}:{LAYER_LABELS[l]}</button>
          ))}
          <Div/>
          <button onClick={fillGems} style={{
            background:"#1a2b00",border:"1px solid #4ade80",color:"#4ade80",
            padding:"5px 12px",fontSize:11,cursor:"pointer",letterSpacing:1,fontFamily:"inherit",fontWeight:700}}
          ...
          <button onClick={clearGems} style={{
            ...
            padding:"5px 12px",fontSize:11,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"}}
        ...
        <button onClick={simMode?exitSimulator:enterSimulator} style={{
          ...
          padding:"5px 14px",fontSize:11,cursor:"pointer",letterSpacing:2,fontWeight:700,fontFamily:"inherit"}}>
          {simMode?"◼ EXIT SIM":"▶ SIMULATE"}
        </button>

        {simMode&&<>
          <span style={{fontSize:12,color:"#e6edf3"}}>
            MOVES <span style={{color:simMoves<=5?"#ef4444":"#facc15",fontWeight:700,fontSize:14}}>{simMoves}</span>
          </span>
          <span style={{fontSize:12,color:"#e6edf3"}}>
            SCORE <span style={{color:"#4ade80",fontWeight:700,fontSize:14}}>{simScore}</span>
          </span>
          {gravPhase!=="idle"&&
      ...
      {notification&&<div style={{position:"fixed",top:44,left:"50%",transform:"translateX(-50%)",
        ...
        color:"#e6edf3",padding:"6px 16px",fontSize:12,letterSpacing:1,zIndex:9999}}>{notification.msg}</div>}

      {/* MAIN */}
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>

        {/* LEFT */}
        <aside style={{width:260,background:"#0d1117",borderRight:"1px solid #21262d",
          ...
          <div style={{display:"flex",borderBottom:"1px solid #21262d"}}>
            {["library","presets","levels"].map(tab=>(
              <button key={tab} onClick={()=>setActiveTab(tab)} style={{
                ...
                padding:"9px 2px",fontSize:10,cursor:"pointer",letterSpacing:1,fontFamily:"inherit",textTransform:"uppercase"}}>{tab}</button>
            ))}
          </div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:2,marginBottom:5}}>
                {categories.map(c=>(
                  <button key={c} onClick={()=>setActiveCategory(c)} style={{
                    ...
                    padding:"3px 8px",fontSize:9,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"}}>{c.toUpperCase()}</button>
                ))}
              </div>
              {filteredBlockers.map(b=>(
                <div key={b.id} onClick={()=>{if(!simMode){setSelectedBlockerId(b.id);setTool("brush");}}}
                  style={{display:"flex",alignItems:"center",gap:7,padding:"7px 8px",marginBottom:3,
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:11,color:"#e6edf3"}}>{b.name}</div>
                    <div style={{fontSize:9,color:"#6e7681"}}>L{b.layer}|HP:{b.hp}|TAX:{b.turnTax}</div>
                  </div>
              ...
              {!simMode&&(newBlockerForm
                ...
                : <button onClick={()=>setNewBlockerForm({id:"",name:"",layer:1,hp:2,posEdge:true,dmgType:"Color",gravityImpact:false,turnTax:2,color:"#888888",icon:"?",category:"blocker"})}
                    style={{width:"100%",marginTop:6,background:"transparent",border:"1px dashed #30363d",
                      color:"#58a6ff",padding:"8px",fontSize:11,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"}}>+ NEW BLOCKER</button>
              )}
            </>}
            {activeTab==="presets"&&PRESETS.map(p=>(
              <div key={p.id} onClick={()=>loadLevel(p)}
                style={{padding:"8px 10px",cursor:"pointer",border:"1px solid #21262d",marginBottom:3,background:"transparent"}}
                onMouseEnter={e=>e.currentTarget.style.background="#161b22"}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <div style={{fontSize:11,color:"#e6edf3"}}>{p.name}</div>
                <div style={{fontSize:9,color:"#6e7681"}}>CH{p.chapter}·{p.moves}mv</div>
              </div>
            ))}
            {activeTab==="levels"&&(savedLevels.length===0
              ...
              : savedLevels.map((lvl,i)=>(
                <div key={i} onClick={()=>loadLevel(lvl)}
                  ...
                  onMouseEnter={e=>e.currentTarget.style.background="#161b22"}
                  onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{fontSize:11,color:"#e6edf3"}}>{lvl.name}</div>
                  <div style={{fontSize:9,color:"#6e7681"}}>CH{lvl.chapter}·{lvl.moves}mv</div>
                </div>
              ))
            )}
          </div>
        ...
        <main style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
            {simMode?(
              <>
                <span style={{fontSize:10,color:"#6e7681",letterSpacing:0.5}}>CLICK=SELECT/SWAP</span>
                <span style={{fontSize:10,color:"#6e7681"}}>·</span>
                <span style={{fontSize:10,color:"#6e7681",letterSpacing:0.5}}>RIGHT-CLICK=ACTIVATE POWERUP</span>
                <span style={{fontSize:10,color:"#58a6ff",letterSpacing:0.5}}>■ SELECTED</span>
                <span style={{fontSize:10,color:"#facc15",letterSpacing:0.5}}>■ MATCH FLASH</span>
              </>
            ):Object.entries(LAYER_LABELS).map(([l,label])=>(
              <span key={l} style={{fontSize:10,color:selectedLayer===Number(l)?"#58a6ff":"#6e7681",letterSpacing:0.5}}>
                <span style={{display:"inline-block",width:9,height:9,background:LAYER_COLORS[l],marginRight:2,verticalAlign:"middle"}}/>
                L{l}:{label}
              </span>
            ))}
          </div>
          ...
          {/* Sim log */}
          {simMode&&<div style={{width:"100%",maxWidth:440,maxHeight:90,overflowY:"auto",
            background:"#0d1117",border:"1px solid #21262d",padding:"3px 8px",boxSizing:"border-box"}}>
            {logLines.map((l,i)=>(
              <div key={i} style={{fontSize:10,color:i===0?"#c9d1d9":"#6e7681",letterSpacing:0.3,lineHeight:1.6}}>{l}</div>
            ))}
          </div>}
        </main>

        {/* RIGHT */}
        <aside style={{width:300,background:"#0d1117",borderLeft:"1px solid #21262d",
          display:"flex",flexDirection:"column",overflow:"hidden"}}>

          <div style={{padding:"11px 14px",borderBottom:"1px solid #21262d"}}>
          ...
          <div style={{padding:"11px 14px",borderBottom:"1px solid #21262d"}}>
            ...
            {objectives.map((obj,i)=>(
              <div key={i} style={{display:"flex",gap:3,alignItems:"center",marginBottom:3}}>
                ...
                <button onClick={()=>setObjectives(objectives.filter((_,j)=>j!==i))}
                  style={{background:"transparent",border:"1px solid #30363d",color:"#ef4444",width:20,height:20,fontSize:11,cursor:"pointer",padding:0,fontFamily:"inherit"}}>×</button>
              </div>
            ))}
            <button onClick={()=>setObjectives([...objectives,{blockerId:"b_stone",count:10}])}
              style={{background:"transparent",border:"1px dashed #30363d",color:"#58a6ff",
                padding:"5px 8px",fontSize:10,cursor:"pointer",width:"100%",letterSpacing:1,fontFamily:"inherit"}}>+ ADD OBJECTIVE</button>
          </div>

          <div style={{padding:"11px 14px",borderBottom:"1px solid #21262d"}}>
            <STitle>PACING CURVE</STitle>
            <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:6}}>
              <span style={{fontSize:10,color:"#8b949e",letterSpacing:1}}>ER</span>
              <span style={{fontSize:13,fontWeight:700,color:erColor,border:`1px solid ${erColor}`,padding:"3px 7px"}}>{er.toFixed(2)}</span>
              <span style={{fontSize:10,color:erColor,letterSpacing:1}}>{erStatus}</span>
              <span style={{marginLeft:"auto",fontSize:10,color:"#6e7681"}}>TAX:<span style={{color:"#e6edf3"}}>{totalTax.toFixed(1)}</span>mv</span>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
              <span style={{fontSize:10,color:"#58a6ff"}}>── REAL</span>
              <span style={{fontSize:10,color:"#f72585"}}>╌╌ TARGET</span>
              <span style={{fontSize:9,color:"#6e7681"}}>Y=moves needed</span>
            </div>
            <ResponsiveContainer width="100%" height={110}>
              <LineChart data={chartData} margin={{top:2,right:2,left:-10,bottom:2}}>
                <XAxis dataKey="movesRemaining" tick={{fontSize:8,fill:"#6e7681"}}/>
            ...
            <div style={{fontSize:9,color:"#6e7681",marginTop:4,lineHeight:1.8}}>
            ...
            {targetCurveInput.map((val,i)=>{
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:3,marginBottom:2}}>
                  <span style={{fontSize:9,color:"#6e7681",width:16,textAlign:"right"}}>T{i}</span>
                  ...
                  <input type="number" min={0} step={0.5} value={val}
                    onChange={e=>{const n=[...targetCurveInput];n[i]=+e.target.value;setTargetCurveInput(n);}}
                    style={{...IS,width:32,padding:"1px 3px",fontSize:10,color:"#f72585"}}/>
                  <span style={{fontSize:9,color:"#6e7681"}}>mv</span>
                </div>
              );
            })}
            <div style={{display:"flex",gap:3,marginTop:4}}>
              {[["+ PT","#58a6ff",()=>setTargetCurveInput(p=>[...p,0])],
                ...
                <button key={lbl} onClick={fn} style={{flex:1,background:"transparent",border:`1px dashed #30363d`,
                  color:col,fontSize:10,cursor:"pointer",padding:"5px",fontFamily:"inherit"}}>{lbl}</button>
              ))}
            </div>
          </div>
            ...
            {boardSnapshot.length===0
              ...
              : <>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:"1px 5px",marginBottom:3}}>
                    <span style={{fontSize:9,color:"#6e7681",letterSpacing:1}}>BLOCKER</span>
                    <span style={{fontSize:9,color:"#6e7681",letterSpacing:1}}>POS</span>
                    <span style={{fontSize:9,color:"#58a6ff",letterSpacing:1,textAlign:"right"}}>TAX</span>
                  </div>
                  {boardSnapshot.map((it,idx)=>(
                    <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:"0 5px",
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:3}}>
                        ...
                        <span style={{fontSize:10,color:"#c9d1d9"}}>{it.name}</span>
                        <span style={{fontSize:9,color:"#6e7681"}}>L{it.layer}</span>
                      </div>
                      <span style={{fontSize:9,color:"#6e7681"}}>{it.x},{it.y}</span>
                      <span style={{fontSize:11,color:"#58a6ff",fontWeight:700,textAlign:"right"}}>
                        {it.turnTax}<span style={{fontSize:8,color:"#6e7681",fontWeight:400}}>mv</span>
                      </span>
                    </div>
                  ))}
                  <div style={{marginTop:5,padding:"3px 5px",background:"#161b22",border:"1px solid #21262d",
                    display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:9,color:"#8b949e",letterSpacing:1}}>TOTAL</span>
                    <span style={{fontSize:13,color:"#58a6ff",fontWeight:700}}>{totalTax.toFixed(1)} mv</span>
                  </div>
                </>
            }
          </div>
              ...
              : <>
                  ...
                  {mainB&&
                    <div style={{position:"relative",zIndex:2,textAlign:"center",lineHeight:1}}>
                      ...
                      {/* Gem color stripe */}
                      {mainB.category==="gem"&&
                        <div style={{width:30,height:4,background:mainB.color+"dd",margin:"2px auto 0",borderRadius:2}}/>}
                      {/* PU type badge */}
                      {mainB.category==="powerup"&&
                        <div style={{fontSize:8,color:mainB.color,letterSpacing:0.5,marginTop:2,fontWeight:700}}>
                  ...
                  {cover&&(simMode||selectedLayer!==1)&&
                    <div style={{position:"absolute",top:1,right:1,zIndex:3,lineHeight:1,
                      ...
                      <span style={{fontSize:simMode?15:11}}>{cover.icon}</span>
                  ...
                  {overlay&&(simMode||selectedLayer!==2)&&
                    <div style={{position:"absolute",bottom:1,right:1,zIndex:3,opacity:simMode?1:0.7}}>
                      <span style={{fontSize:simMode?15:11}}>{overlay.icon}</span>
                    </div>}
                  ...
                  {/* Coords */}
                  <div style={{position:"absolute",bottom:0,left:1,fontSize:7,color:"#30363d",lineHeight:1}}>{x},{y}</div>
                </>}
          </div>
        );
      }))}
...
function BlockerForm({form,onChange,onConfirm,onCancel}){
  ...
  return(
    <div style={{border:"1px solid #21262d",padding:7,marginTop:6,background:"#0d1117"}}>
      <div style={{fontSize:10,color:"#58a6ff",letterSpacing:2,marginBottom:7}}>NEW BLOCKER</div>
...
const STitle=({children})=>(
  <div style={{fontSize:10,letterSpacing:2,color:"#8b949e",marginBottom:7,fontWeight:700}}>{children}</div>
);
...
const Div=()=><div style={{width:1,height:22,background:"#21262d",flexShrink:0}}/>;
const IS={background:"#161b22",border:"1px solid #30363d",color:"#c9d1d9",
  padding:"5px 8px",fontSize:11,width:"100%",boxSizing:"border-box",fontFamily:"inherit",outline:"none"};
const BTN=bg=>({background:bg,border:"none",color:"#e6edf3",
  padding:"5px 12px",fontSize:10,cursor:"pointer",letterSpacing:1,fontFamily:"inherit"});
